...
class Net(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.layers = torch.nn.Sequential(torch.nn.Linear(784,200),
                                          torch.nn.Sigmoid(),
...
class MNIST(Dataset):
    def __init__(self, train=True):
        ...
        self.x_train = torch.from_numpy(dataset['train_img']/255.).float()
        self.y_train = torch.from_numpy(dataset['train_label']).long()
        self.x_test  = torch.from_numpy(dataset['test_img']/255.).float()
        self.y_test  = torch.from_numpy(dataset['test_label']).long()
