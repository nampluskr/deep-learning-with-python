...
from numpy_nn_lib import onehot_encode, sigmoid, softmax, cross_entropy
...
if __name__ == "__main__":
    ...
    y_train = onehot_encode(mnist['train_label'], 10)
    x_test  = mnist['test_img']/255.
    y_test  = onehot_encode(mnist['test_label'], 10)
    ...
    for epoch in range(n_epoch):
        ...
        for i in range(n_batch):
            ...
            loss_batch = cross_entropy(softmax(output), target)
            acc_batch = (softmax(output).argmax(1) == target.argmax(1)).mean()
    ...
    sig1 = sigmoid(np.dot(x_test, w1) + b1)
    pred = softmax(np.dot(sig1, w2) + b2)

    loss_test = cross_entropy(pred, y_test)
